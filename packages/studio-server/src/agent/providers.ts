...
- To put a one-line caption onto two lines at its full size, three things together, all on that caption: remove its \`data-caption-base-px\`, add \`style="flex-wrap: wrap;"\` to the same element (this is the one exception to the "nothing else" above), and insert a break element \`<div class="hf-caption-break"></div>\` between the words where the line should end — after the fourth word for four words per line. The break must be a literal element written into the file, because Studio can only patch elements that already exist; it must be a \`div\` and not a \`span\`, so the word-counting scripts skip it. Leave every other caption's attribute alone unless the user asks for the same change there.
