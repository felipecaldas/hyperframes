 ...
 *
 * The comparison is spelled out rather than left as two numbers, because the
 * question the caption instruction asks is "does this text fit" and a width on
 * its own does not answer it — the model would have to be handed the box as
 * well and do the subtraction. A clause also keeps `describeMeasuredElement`
 * under the fork's complexity gate, which two more branches there would exceed.
 */
function lineWidthClause(el: LayoutElementMeasurement): string | null {
  if (typeof el.widestLinePx !== "number" || typeof el.contentBoxPx !== "number") return null;
  const over = Math.round(el.widestLinePx - el.contentBoxPx);
  return (
    `widest line ${el.widestLinePx}px in a ${el.contentBoxPx}px content box` +
    (over > 1 ? `, ${over}px too wide` : ", within it")
  );
}
