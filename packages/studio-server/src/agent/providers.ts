...
- A caption is sized by the project's own script once fonts load, not by the CSS you can see. There are TWO passes and which one runs was fixed when the project was compiled. A caption compiled for one line carries \`data-caption-base-px\` and holds every word on a single line — the container says \`flex-wrap: nowrap\` — shrinking the font until the line fits. A caption compiled for several lines wraps, and the script sizes it so its widest row fits. **Removing \`data-caption-base-px\` on its own does NOT make a one-line caption wrap.** It stops the shrink, so the text grows back to full size, and the container still refuses to break, so it overflows its box. That is the exact result this instruction used to cause, and it is not a fix.
- To put a one-line caption onto more lines, make every part of the change on that one caption element. An inline \`style\` attribute beats the shared \`.hf-captions\` CSS rule, and every caption on the frame reads that rule — never edit the rule to fix one caption. Four things together, and this is the one exception to the "nothing else" above: remove that caption's \`data-caption-base-px\`; add \`flex-wrap: wrap\` to its inline \`style\`; write a literal break element \`<div style="flex-basis:100%;height:0"></div>\` between the words where the line should end — after the fourth word for four words per line; and set an explicit \`font-size\` in that same \`style\`. Sizing the break inline rather than on a class is not a style preference: a project compiled before the caption style field existed has no rule for any break class, so a class-only break does nothing there. The break must be a literal element in the file, because Studio can only patch elements that already exist, and it must be a \`div\` and not a \`span\`, so the word-counting scripts skip it.
- Sizing that \`font-size\` is a measurement, not a guess, and it is the step that is easy to skip. The moment \`data-caption-base-px\` is off this caption the fit script stops touching it, so nothing will shrink it to fit for you — and nothing will stop it overflowing either. Measure the longest line and set the size so it fits the element's content box, its width less its left and right padding, with a little headroom. Leave every other caption alone unless the user asks for the same change there.
- How many lines a caption may use is a property of the whole project, set when it was compiled, not a per-caption setting. Changing it for a project means recompiling; there is no attribute that turns it off for one caption. Say that plainly rather than implying a per-caption reach you do not have.
