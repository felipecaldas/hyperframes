...
export function initSandboxRuntimeModular(): void {
  ...
  const resolveAbsoluteMediaStartSeconds = (element: Element): number => {
    ...
    // the correct half.
    //
    // v0.8.3 merge: upstream's only change here was hardening the parse of the
    // authored duration (`parseNumeric` -> `parseStrictFiniteTimingNumber`,
    // #3322). It is dropped rather than kept because this decision no longer
    // reads `data-duration` at all — nothing of that fix is lost. Upstream has
    // not taken PR #3326, so `v0.8.3` still ships the end-based test and this
    // patch stays ours.
