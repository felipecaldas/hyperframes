...
describe("external file-change subscription ownership", () => {
  ...
  it.each([2, 0])(
    "stops a failed stream and ignores late deliveries (readyState=%s)",
    (readyState) => {
      const listeners = new Map<string, (event: unknown) => void>();
      const close = vi.fn();
      ...
      try {
        const onDelivery = vi.fn();
        const stop = sseFileChangeChannel(onDelivery);
        const failures = readyState === 2 ? 1 : 4;
        ...
        listeners.get("file-change")?.(new MessageEvent("file-change", { data: "{}" }));
